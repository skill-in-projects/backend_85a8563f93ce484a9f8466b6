...
async function sendErrorToEndpoint(endpointUrl, boardId, req, error) {
    try {
        ...
        for (let i = 1; i < stackLines.length; i++) {
            ...
            // Match patterns like: "at getAll (/app/Controllers/TestController.js:43:11)"
            // or: "at /app/app.js:57:25"
            // First try: "at functionName (file:line:column)"
            ...
            // Second try: "at file:line:column" (no function name)
            fileMatch = line.match(/at\s+([^:]+):(\d+):(\d+)/);
            if (fileMatch) {
